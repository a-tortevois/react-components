...
const ArcProgressBar = props => {
  ...
  // Fix value to 0 -> 100%
  const value =
    props.value < 0 ? 0 : props.value > 100 ? 100 : props.value || 0;

  // Fix diameter to 100 -> 250
  const diameter =
    props.diameter < 100 ? 100 : props.diameter > 250 ? 250 : props.diameter;
  ...
  return (
    <div>
      <svg className='Arc-progress-bar' width={diameter} height={diameter}>
        <path
        ...
        ></path>
        <text className='value' x={center} y={center}>
          {value}%
        </text>
      </svg>
    </div>
